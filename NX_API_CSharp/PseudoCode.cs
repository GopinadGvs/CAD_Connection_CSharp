...
namespace NXAPI
{
    internal class PseudoCode
    {
        ...
        //{
        //uI.SelectionManager.SelectTaggedObject("Select a face", "Select a face to create a point feature", Selection.SelectionScope.WorkPart, false, false, out TaggedObject taggedObject, out Point3d cursor);

        //int[] blocks = new int[5];

        //uFSession.Drf.AskNumberBlocks(taggedObject.Tag, blocks);

        //(taggedObject as Dimension).GetDimensionText(out string[] xx, out string[] yy);


        //AppendedText text = (taggedObject as Dimension).GetAppendedText();

        //string[] val1 = text.GetAboveText();
        //string[] val2 = text.GetBeforeText();
        //string[] val3 = text.GetAfterText();
        //string[] val4 = text.GetBelowText();


        //Part workPart = theSession.Parts.Work;

        //uFSession.Draw.AskCurrentDrawing(out Tag drawing_tag);

        //uFSession.Draw.AskViews(drawing_tag, out int view_count, out Tag[] view_tags);

        //uFSession.Draw.AskDisplayedObjects(view_tags[0], out int num_objects, out Tag[] objects);


        //DraftingView[] views = theSession.Parts.Work.DraftingDrawingSheets.CurrentDrawingSheet.GetDraftingViews();


        //DraftingDrawingSheet currentSheet = null;
        ////currentSheet.get

        //DisplayableObject[] displayableObjects = views[0].AskVisibleObjects();


        //Dimension[] dimensions =  theSession.Parts.Work.Dimensions.ToArray();

        //theSession.Parts.NewBaseDisplay("E:\\Personal\\NX\\NX-Testing\\01_Data\\F5800010214830101_20.prt", BasePart.Units.Millimeters);

        //theSession.Parts.FileNew();

        //uI.SelectionManager.SelectTaggedObject("Select a face", "Select a face to create a point feature", Selection.SelectionScope.WorkPart, false, false, out TaggedObject taggedObject, out Point3d cursor);

        //Selection.SelectionType[] selectionTypes = new Selection.SelectionType[1];
        //selectionTypes[0] = Selection.SelectionType.Faces;

        //uI.SelectionManager.SelectTaggedObject("Select a face", "Select a face to create a point feature", Selection.SelectionScope.WorkPart, false, selectionTypes, out TaggedObject taggedObject, out Point3d cursor);

        //Selection.MaskTriple maskTriple = new Selection.MaskTriple();
        //maskTriple.Type = UFConstants.UF_face_type;
        //maskTriple.Subtype = UFConstants.UF_plane_type;
        //maskTriple.SolidBodySubtype = 0;

        //maskTriple.Type = UFConstants.UF_solid_type;       // Solid entity
        //maskTriple.Subtype = UFConstants.UF_solid_face_subtype;  // Solid face subtype
        ////maskTriple.SolidBodySubtype = UFConstants.UF_UI_SEL_FEATURE_PLANAR_FACE; // Planar Face subtype
        //maskTriple.SolidBodySubtype = UFConstants.UF_UI_SEL_FEATURE_CYLINDRICAL_FACE; // Planar Face subtype

        //Selection.MaskTriple[] maskTriples = new Selection.MaskTriple[] { maskTriple };            

        //uI.SelectionManager.SelectTaggedObject("Select a face", "Select a face to create a point feature", Selection.SelectionScope.WorkPart, Selection.SelectionAction.ClearAndEnableSpecific, false, false, maskTriples, out TaggedObject taggedObject, out Point3d cursor);

        //Point point = theSession.Parts.Work.Points.CreatePoint(new Point3d(0, 0, 10), SmartObject.UpdateOption.WithinModeling);

        //NXOpen.Features.Feature nullNXOpen_Features_Feature = null;

        //PointFeatureBuilder pointFeatureBuilder = theSession.Parts.Work.BaseFeatures.CreatePointFeatureBuilder(nullNXOpen_Features_Feature);
        ...
        //cornerPoint[0] = 0.0; // X coordinate  
        //cornerPoint[1] = 0.0; // Y coordinate
        //cornerPoint[2] = 0.0; // Z coordinate

        //string[] edgeLens = new string[3];
        //edgeLens[0] = "100"; // Length of edge 1
        //edgeLens[1] = "100"; // Length of edge 2
        //edgeLens[2] = "100"; // Length of edge 3

        //uFSession.Modl.CreateBlock(FeatureSigns.Nullsign, Tag.Null, cornerPoint, edgeLens, out Tag blockTag);

        //uFSession.Curve.CreatePoint(new double[] { 10, 10, 10 }, out Tag pointTag);

        //uFSession.Modl.CreatePointsFeature(1, new Tag[] { pointTag }, out Tag pointFeatTag);



        //uFSession.Obj.SetLayer(blockTag, 10);

        //string partName = theSession.Parts.Work.Name;

        //string displayPartName = theSession.Parts.Display.Name;

        //Tag partTag = uFSession.Part.AskPartTag(partName);
        //}        
